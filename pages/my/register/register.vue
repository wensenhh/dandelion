...
<script>
	...
	export default {
		...
		onLoad(option) {
			this.registerInfo.invitation=option.invite_code ?option.invite_code:""
			this.getLeader()
		},
		methods: {
			getLeader() {
				this.tui.request("/account/leader", "POST", {
					invitation: this.registerInfo.invitation
				}).then((res) => {
			...
			async sumbit() {
				...
				if (!checkRes) {
					this.tui.toast('提交成功');
					const res = await register(this.registerInfo);
						console.log(res)
						if (res.code == 1) {
							this.tui.toast('提交成功');
