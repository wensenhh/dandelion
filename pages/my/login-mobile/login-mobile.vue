			...
			<view class="info-box">
				...
				<view class="">
					<view class="div_style">
						...
						<input type="number" v-model="loginInfo.account" name="account" placeholder="请输入手机号" />
				...
				<view class="withdrawalStyle" style="border-bottom: 2rpx solid #EEEEEE; ">
					<view class="div_style">
						...
						<input type="text" v-model="loginInfo.code" placeholder="请输入手机验证码" />
...
<script>
	...
	import {smsCodeLogin} from "@/api/app.js"
	export default {
		data() {
			return {
				show: true,
		...
		async	sumbit() {
				...
				const res = await smsCodeLogin(this.loginInfo)
					if(res.code==1){
			...
			getCode() {
				...
				if (!checkRes) {
					...
					this.tui.request("/sms/send","POST",{mobile:this.loginInfo.account,key:"YZMDL"}).then((res)=>{
					if(res.code==1){
							this.tui.toast(res.msg);
					}else{
						if (res.show == 1) {
