		...
		<view class="tui-header-box">
			<view class="tui-header" :style="{ width: width + 'px', height: height + 'px' }">
				...
				<view class="tui-searchbox tui-search-mr" :style="{ marginTop: inputTop + 'px' }">
					<input type="text" v-model="goodsListParam.keyword" placeholder="搜索商品标题"
		...
		<view class="tui-product-list" :style="{ marginTop: px(dropScreenH + 18) }" v-else>
			<view class="tui-product-container">
				<block v-for="(item, index) in productList" :key="index" v-if="(index + 1) % 2 != 0 || isList">
					<view class="ye">
						<view class="ye-text" v-if="activityId ==2">
							{{item.discount}}折
						</view>
						<view class="ye-text" v-else>
							{{label}}
						</view>
					</view>
					<!--商品列表-->
							...
							<view>
								<view class="tui-pro-price">
									<text class="tui-sale-price"
										v-if="item.findWholesale==1?true:false">{{item.price }}</text>
...
this.goodsListParam.id=id;
		...
		methods: {
			...
			async getActivityGoodsListsFun() {
				...
				const data = await loadingFun(getActivityGoodsLists, page, productList, status, this.goodsListParam)
				if (!data) return
				...
				console.log("data.data",data.data,data.data.label)
			},
			onPullDownRefresh() {
				this.reflesh()
				setTimeout(() => {
