		...
		methods: {
			...
			sub(storeIndex, goodsIndex, goodsnum){
				...
				if(goodsnum == 1){
					...
					this.addsubcard(this.datas[storeIndex].goods[goodsIndex].id,2,function(){
						that.datas[storeIndex].goods[goodsIndex].number--
						that.statistics()
					})
				}
			},
			...
			add(storeIndex, goodsIndex, goodsnum){
				let that = this
				console.log(this.datas[storeIndex].goods[goodsIndex])
				this.addsubcard(this.datas[storeIndex].goods[goodsIndex].id,1,function(){
					that.datas[storeIndex].goods[goodsIndex].number++
					that.statistics()
				})
			},
			//增加减少接口
			addsubcard(id,type,callback){
				this.$request.post('/cart/updateNumCart',{
					cart_id: id,
					type: type
				}).then(res => {
					if(res.code == 1){
